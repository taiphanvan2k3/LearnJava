package File_p7.test;

import java.io.File;
import java.io.IOException;
import java.util.Scanner;

import File_p7.Main.DanhSachSv;
import File_p7.Main.SinhVien;
...
public class writeObject {
	public static void main(String[] args) throws ClassNotFoundException, IOException {
		...
		while (true) {
			...
			System.out.println("10.Ghi toàn bộ dữ liệu xuống file");
			System.out.println("11.Đọc toàn bộ dữ liệu từ file");
			...
			if (lc == 0)
				...
				danhSach.sapXepGiamDanGpa2();
				danhSach.inDanhSach();
			} else if(lc==10){
				System.out.println("Nhập tên file:");
				//Chỉ cần nhập tên file
				String s=sc.next();
				File f=new File(s);
				danhSach.writeIntoFile(f);
			}else if(lc==11) {
				System.out.println("Nhập tên file:");
				//Chỉ cần nhập tên file
				String s=sc.next();
				File f=new File(s);
				danhSach.readFromFile(f);
			}
		}
	}
